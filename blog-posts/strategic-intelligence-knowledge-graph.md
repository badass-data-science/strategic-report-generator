...
The first thing to get straight, because our heroine spent an embarrassing amount of time getting it straight in her own head first, is that this pipeline now produces *two different graphs*, and they are not the same graph wearing different outfits. They answer different questions for different audiences, and one of our heroine's few hard rules on this project is that neither is allowed to pretend to be the other.

The first graph is the one you can actually look at: a tag co-occurrence network, one node per tag, one edge per pair of tags that showed up in the same article, weighted by how often that happened. It gets rebuilt fresh every single night from that night's articles alone, run through NetworkX, and rendered as a self-contained, zero-dependency D3.js force-directed graph that opens straight from your filesystem — no web server, no build step, just double-click and there's the drama. Node size is article count, edge thickness is co-occurrence strength, and color comes from something more interesting than "the biggest topic wins," which brings us to algorithm number one.
