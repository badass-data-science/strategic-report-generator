...
- **No RDF export of any of this.** `export-rdf` walks the same tracking database — urgency scores, tag and community structure, bridge tags, cross-topic overviews — and turns it into a portable RDF knowledge graph, but it has no idea Systems Signals exists. A candidate feedback loop currently lives and dies inside one rendered `index.html`; it never becomes a fact in the graph the way a bridge tag or a community summary does. Doing this properly means a real vocabulary decision first — a lagged correlation isn't a `skos:Concept` or a `prov:Activity`, it's closer to a structured claim *about* two other facts, which probably means a new `stratrep:` term relating two topics or tags with a lag, a correlation coefficient, and a q-value — and it inherits the previous bullet's open question besides: compute it fresh at export time, the same way the report itself works today, or only once there's something durable worth reading.
