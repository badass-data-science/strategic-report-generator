...
def _seed(db_path, run_id: str, article_count: int, communities: dict) -> None:
...
class TestFindRelevantCommunities:
    def test_empty_candidate_tags_returns_empty(self, db_path):
        _seed(db_path, "run-0", 10, {
            0: {"label": "policy", "tags": ["policy"], "summary": "Coverage of policy.", "article_count": 1},
        })
        assert find_relevant_communities(db_path, []) == []

    def test_no_matches_returns_empty(self, db_path):
        _seed(db_path, "run-0", 10, {
            0: {"label": "policy", "tags": ["policy"], "summary": "Coverage of policy.", "article_count": 1},
        })
        assert find_relevant_communities(db_path, ["unrelated-topic"]) == []

    def test_exact_tag_membership_match(self, db_path):
        _seed(db_path, "run-0", 10, {
    ...
    def test_substring_fallback_on_summary_text(self, db_path):
        """A candidate tag with no exact tag-membership hit still matches via summary text."""
        _seed(db_path, "run-0", 10, {
            0: {"label": "biotech", "tags": ["genomics"],
                "summary": "New gene editing breakthroughs announced this week.", "article_count": 2},
        })
        results = find_relevant_communities(db_path, ["gene editing"])
        assert len(results) == 1
        assert results[0]["label"] == "biotech"

    def test_dedup_across_multiple_matching_tags(self, db_path):
    ...
    def test_most_recent_first(self, db_path):
        _seed(db_path, "run-0", 10, {
            0: {"label": "policy", "tags": ["policy"], "summary": "Older coverage.", "article_count": 1},
        })
        _seed(db_path, "run-1", 10, {
            0: {"label": "policy", "tags": ["policy"], "summary": "Newer coverage.", "article_count": 1},
        })
    ...
    def test_limit_caps_results(self, db_path):
        for i in range(5):
            _seed(db_path, f"run-{i}", 10, {
                0: {"label": "policy", "tags": ["policy"], "summary": f"Coverage {i}.", "article_count": 1},
            })
    ...
    def test_returns_expected_fields(self, db_path):
        _seed(db_path, "run-0", 10, {
            0: {"label": "policy", "tags": ["policy"], "summary": "Coverage of policy.", "article_count": 7},
        })
