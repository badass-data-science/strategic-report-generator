...
def _append_n_runs(db_path: Path, topic: str, scores: list[float]) -> None:
...
class TestAbsoluteThreshold:
    def test_no_alert_no_history_below_threshold(self, db_path: Path) -> None:
        alerts = check_alerts(
            [_make_result("AI", 0.5)],
    ...
    def test_alert_no_history_above_threshold(self, db_path: Path) -> None:
        alerts = check_alerts(
            [_make_result("AI", 0.9)],
            load_history(db_path),
            absolute_threshold=0.8,
    ...
    def test_alert_thin_history_above_threshold(self, db_path: Path) -> None:
    ...
    def test_no_alert_thin_history_below_threshold(self, db_path: Path) -> None:
        _append_n_runs(db_path, "AI", [0.3, 0.4, 0.35, 0.42, 0.38, 0.41])
        alerts = check_alerts(
            [_make_result("AI", 0.6)],
            load_history(db_path),
    ...
    def test_absolute_fallback_when_std_zero(self, db_path: Path) -> None:
        """
    ...
    def test_no_alert_std_zero_below_absolute(self, db_path: Path) -> None:
...
class TestStatisticalAlert:
    ...
    def test_statistical_no_alert_within_band(self, db_path: Path) -> None:
    ...
    def test_statistical_check_skips_absolute(self, db_path: Path) -> None:
        """
    ...
    def test_custom_z_threshold(self, db_path: Path) -> None:
...
class TestMultiTopic:
    def test_only_alerting_topic_returned(self, db_path: Path) -> None:
        results = [
    ...
    def test_multiple_topics_can_alert(self, db_path: Path) -> None:
        results = [
            _make_result("AI", 0.9),
    ...
    def test_topic_missing_from_history_uses_absolute(self, db_path: Path) -> None:
    ...
    def test_topic_without_strategy_ignored(self, db_path: Path) -> None:
...
class TestHistoryPersistence:
    def test_load_empty_when_no_prior_runs(self, db_path: Path) -> None:
        history = load_history(db_path)
        assert history == {}

    def test_append_creates_db_file(self, db_path: Path) -> None:
    ...
    def test_multiple_appends_accumulate(self, db_path: Path) -> None:
    ...
    def test_ordering_current_run_not_in_own_baseline(self, db_path: Path) -> None:
        """
