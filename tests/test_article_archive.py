...
class TestRecordAndLoadArticles:
    def test_round_trip_single_article(self, db_path: Path) -> None:
    ...
    def test_bullet_order_preserved(self, db_path: Path) -> None:
        record_run(db_path, "run-0", article_count=1)
    ...
    def test_tag_order_preserved(self, db_path: Path) -> None:
        record_run(db_path, "run-0", article_count=1)
    ...
    def test_multiple_articles_same_topic(self, db_path: Path) -> None:
    ...
    def test_multiple_topics(self, db_path: Path) -> None:
        record_run(db_path, "run-0", article_count=2)
    ...
    def test_error_topic_contributes_nothing(self, db_path: Path) -> None:
    ...
    def test_empty_topic_contributes_nothing(self, db_path: Path) -> None:
        record_run(db_path, "run-0", article_count=0)
        config = TopicConfig(
            slug="feeds_ai", title="Artificial Intelligence", feeds_file=Path("/dev/null")
        )
    ...
    def test_unknown_run_id_returns_empty(self, db_path: Path) -> None:
        record_run(db_path, "run-0", article_count=1)
