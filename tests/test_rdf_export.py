...
from strategic_reports.daily.core.models import ArticleSummary, StrategicInsight, TopicConfig, TopicResult
...
def _make_result(topic_title: str = "Artificial Intelligence") -> TopicResult:
    ...
    strategy = StrategicInsight(
        bullets=["Invest in LLM tooling now.", "Target applied-AI roles.", "Build small deployed projects."],
        urgency_score=0.42,
    )
    return TopicResult(config=config, articles=[article], strategy=strategy)


def _seed_full_run(db_path, run_id: str, article_count: int = 1) -> None:
    ...
    record_community_summaries(
        db_path,
        run_id,
        {0: {"label": "AI", "tags": _TAGS, "summary": "A cluster about AI progress.", "article_count": 1}},
    )
    record_bridge_tags(
        db_path,
        run_id,
...
class TestBuildGraphArticles:
    def test_article_has_headline_url_and_tags(self, db_path):
        ...
        assert summary_bullets == {"Models are faster.", "Costs are dropping.", "New benchmarks set."}

    def test_article_generated_by_its_run(self, db_path):
        _seed_full_run(db_path, "run-0")
        graph = build_graph(db_path)
...
class TestBuildGraphCommunitiesAndBridgeTags:
    def test_community_is_skos_collection_with_members(self, db_path):
        _seed_full_run(db_path, "run-0")
        graph = build_graph(db_path)
        ...
        member_labels = {str(graph.value(m, SKOS.prefLabel)) for m in graph.objects(comm, SKOS.member)}
        assert member_labels == set(_TAGS)

    def test_bridge_tag_becomes_observation_node(self, db_path):
        _seed_full_run(db_path, "run-0")
        graph = build_graph(db_path)
...
class TestBuildGraphTopicRun:
    def test_urgency_score_and_strategic_bullets_share_topic_run_node(self, db_path):
        _seed_full_run(db_path, "run-0")
        graph = build_graph(db_path)
...
class TestBuildGraphOverview:
    def test_overview_bullet_included(self, db_path):
        _seed_full_run(db_path, "run-0")
        graph = build_graph(db_path)

        overviews = list(graph.subjects(RDF.type, STRATREP.CrossTopicOverview))
        assert len(overviews) == 1
        assert (overviews[0], STRATREP.overviewBullet, Literal("Overview bullet for run-0.")) in graph


class TestSinceFilter:
    def test_full_rebuild_includes_both_runs(self, db_path):
        _seed_full_run(db_path, "run-0")
        _seed_full_run(db_path, "run-1")
        graph = build_graph(db_path)
        assert len(list(graph.subjects(RDF.type, PROV.Activity))) == 2

    def test_since_run_id_excludes_earlier_runs(self, db_path):
        _seed_full_run(db_path, "run-0")
        _seed_full_run(db_path, "run-1")
    ...
    def test_since_with_no_matching_runs_returns_empty_graph(self, db_path):
        _seed_full_run(db_path, "run-0")
...
class TestExportRdf:
    def test_export_writes_parseable_turtle(self, db_path, tmp_path):
        _seed_full_run(db_path, "run-0")
